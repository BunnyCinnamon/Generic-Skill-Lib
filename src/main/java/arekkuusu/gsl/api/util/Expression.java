...
public class Expression {
    ...
    public static final Function<String, FunctionInfo> EXPRESSION_PARSER_SUPPLIER = Expression::parse;
    ...
    public static double getExpression(ResourceLocation location, String function, int min, int max) {
        Int2DoubleArrayMap cache = GSLAPI.EXPRESSION_CACHE.asMap().computeIfAbsent(new Tuple<>(location, function), Expression.EXPRESSION_CACHE_SUPPLIER);
        if(min > max) min = max;
    ...
    public static double getExpression(ResourceLocation location, String[] functionArray, int min, int max) {
        Object2ObjectMap<String, FunctionInfo> cache = GSLAPI.EXPRESSION_FUNCTION_CACHE.asMap().computeIfAbsent(location, Expression.EXPRESSION_FUNCTION_CACHE_SUPPLIER);
        FunctionInfo match = null;
        FunctionInfo temp = null;
        if(min > max) min = max;
        for (String s : functionArray) {
            FunctionInfo info = cache.computeIfAbsent(s, Expression.EXPRESSION_PARSER_SUPPLIER);
            if (info.matches(min) && isNotOverride(temp, info, min)) {
                match = info;
            }
            temp = info;
        }
        return match != null ? Expression.getExpression(location, match.function, min, max) : 0;
    }
    ...
    public static FunctionInfo parse(String string) {
        Pattern pattern = Pattern.compile(Expression.EXPRESSION_REGEX);
        ...
        if (matcher.matches()) {
            ...
            throw new IllegalStateException("[Expression] - Expression " + string + " is not valid, might be missing a { or }");
        }
    }
