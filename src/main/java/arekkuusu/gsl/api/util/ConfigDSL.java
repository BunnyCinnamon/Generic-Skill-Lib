...
public class ConfigDSL {
    ...
    public static class Config {
        ...
        public double get(Skill<?> skill, String name, int lvl, double effective) {
            Int2DoubleArrayMap cache = GSLAPI.EXPRESSION_CACHE.asMap().computeIfAbsent(new Tuple<>(Objects.requireNonNull(skill.getRegistryName()), name), Expression.EXPRESSION_CACHE_SUPPLIER);
            ...
            try {
                ...
                LOGGER.error("Please add this on the modname/.../{}.cfg file", skill.getRegistryName());
                e.printStackTrace();
                result = 0D;
            } finally {
    ...
    public static final BiFunction<Property, String[], Curve> CURVE_RAMP = (property, strings) -> {
        ...
        switch (strings[0]) {
            case RAMP_POSITIVE -> strength = 0.5D;
            case RAMP_NEGATIVE -> strength = -0.5D;
            default -> {
    ...
    public static final Function<String[], Curve> CURVE_FUNCTION = strings -> {
        ...
        return (min, max, start, end, n) -> {
            return Expression.getExpression(new ResourceLocation("dsl"), function, n, max);
        };
    };
    ...
    public static double parseDoubleFromString(Property property, String string) {
        ...
        if (string.contains(REFERENCE)) {
            ...
            return switch (arg) {
                case "start" -> property.start;
                case "end" -> property.end;
                default -> property.map.get(string).end;
            };
        }
